...
//鲜果导航样式初始化
	...
	$navMiddle.find("li").eq(1).css({"border-bottom":"2px solid #64A131"});
	$navMiddle.find("li").eq(1).find("a").css({color:"#64A131"});
	$navMiddle.find("li").eq(1).find("span").css({color:"#64A131"});
	...
	$.ajaxSetup({
		...
		success:function(res){
			...
			$.each(res,function(idx,item){
				...
				$li.attr({"xfruitId":item.xfruitId});
				...
				if(item.condition !=""){
							...
							switch (item.condition){
								case "新品":
									$span1.css({background:"#64A131"});
									break;
								case "包邮":
									$span1.css({background:"skyblue"});
									break;
								case "补货中":
									$span1.css({background:"silver"});
									break;
								default:
									$span1.css({background:"#ff5353"});
									break;
							}
...
//点击购物车弹窗的叉号或继续购物或键盘Enter键，就让弹窗隐藏
	...
	$spanClose.on("click", function() {
		...
		$littleCarLogo.css({backgroundPosition:"-517px -242px"});
	});
